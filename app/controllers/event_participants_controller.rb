class EventParticipantsController < ApplicationController
  ...
  def create
    unless @event.joinable_by?(current_user)
      redirect_to authenticated_root_path, alert: t("events.flash.authorization.inaccessible") and return
    end
    ...
    unless event_team
      redirect_to @event, alert: t("events.flash.teams.not_found") and return
    end

    event_participant.assign_attributes(event_team: event_team)
    if event_participant.save
      redirect_to @event, notice: t("event_participants.flash.create.success", label: event_participant.event_team.label)
    else
  ...
  def destroy
    ...
    if @event_participant.nil?
      redirect_to authenticated_root_path, alert: t("event_participants.flash.destroy.not_found") and return
    end
    ...
    if @event_participant.destroy
      ...
      redirect_to redirect_path, alert: t("event_participants.flash.destroy.success")
    else
      redirect_to @event, alert: t("event_participants.flash.destroy.failure")
    end
  end
