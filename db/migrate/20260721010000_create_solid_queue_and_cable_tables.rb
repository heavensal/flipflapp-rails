...
# Solid Queue + Solid Cable tables live on the primary database (single Neon URL).
# A separate multi-db `queue`/`cable` role on the same URL must not be used:
# `db:schema:load:queue` shares `schema_migrations` and wipes primary versions.
class CreateSolidQueueAndCableTables < ActiveRecord::Migration[8.0]
  def change
  ...
  def add_solid_queue_foreign_keys
    {
      ...
      next if foreign_key_exists?(from_table, to_table, column: :job_id)
